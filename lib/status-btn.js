...
export default class ReloadButtonView {
	...
	updateStatusBarButton( ) {
		...
		var allconfig = atom.config.get('amazon-lambda-ui')

		this.element.classList.add('inline-block');

		if ( allconfig.Layout.StatusBarButtonStyle === 'icon+label')
			this.element.innerHTML = `
				<img height="16" src="atom://amazon-lambda-ui/lambda_favicon.png" />
				Lambda
			`;

		if ( allconfig.Layout.StatusBarButtonStyle === 'icon')
			this.element.innerHTML = `<img height="16" src="atom://amazon-lambda-ui/lambda_favicon.png" />`;
	...
	constructor(statusBar) {
		...
		var $this = this;
		atom.config.observe('amazon-lambda-ui.Layout.StatusBarButtonStyle', function(newValue) {
	...
	onClick(e) {
		e.preventDefault();
		e.stopPropagation();
		atom.commands.dispatch(atom.views.getView(atom.workspace), 'amazon-lambda-ui:new');
	}
